...
qsa('.videoStart').forEach(button => {
  ...
  const restore = () => { button.hidden = false; button.disabled = false; button.setAttribute('aria-label', 'Video abspielen'); };
  button.addEventListener('click', async () => {
    button.disabled = true;
    button.setAttribute('aria-label', 'Video wird geladen');
    try {
      if (video.error) video.load();
      await video.play();
    } catch { restore(); button.setAttribute('aria-label', 'Video erneut abspielen'); }
  });
  video.addEventListener('playing', () => { button.hidden = true; button.disabled = false; });
